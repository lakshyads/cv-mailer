"""
Google Sheets client for reading and updating job application data.
"""
...
class GoogleSheetsClient:
    ...
    def read_all_rows(self, worksheet_name: str = None) -> List[Dict]:
        ...
        try:
            ...
            for i, row in enumerate(values[1:], start=2):  # Start at row 2 (1-indexed)
                row_dict = {
                    header: row[j] if j < len(row) else "" for j, header in enumerate(headers)
                }
    ...
    def update_row(self, row: int, updates: Dict[str, str], worksheet_name: str = None):
        ...
        try:
            ...
            column_map = {
                header.strip().lower(): (chr(65 + i), header) for i, header in enumerate(headers)
            }
